...
contract('farm contract', () => {
    ...
    it('can generate rewards for one staker who claims', async () => {
        const spender = farmContract.instance.address;
        ...
        const delegatorLpAfter = balance.delegatorLpBefore.minus(new BigNumber(depositValue));
        expect(balance.delegatorLpAfter.toFixed()).to.equal(delegatorLpAfter.toFixed());
        
        balance.farmLpAfter = await lpTokenContract.getBalance(farmContract.instance.address);
        const calculatedFarmAfter = balance.farmLpBefore.plus(new BigNumber(depositValue));
        expect(balance.farmLpAfter.toFixed()).to.equal(calculatedFarmAfter.toFixed());
