...
//let accumulatedSTKRPerShare = storage.accumulatedSTKRPerShare + reward * fixedPointAccuracy / contractBalance; // 21 + 18/9=9 *12
    it('', async () => {
      ...
      expect(accumulatedSTKRPerShare).to.equal((new BigNumber('10000000')).multipliedBy(e12).toFixed())
    });
    it.skip('', async () => {
        const accumulatedSTKRPerShare = await _mockContractHelper.updateAccumulatedSTKRperShare(
        ...
        expect(accumulatedSTKRPerShare).to.equal((new BigNumber('8103727')).multipliedBy(e12).toFixed())
      });
    ...
    it('', async () => {
        ...
        expect(accumulatedSTKRPerShare).to.equal(rewardClientFixedPoint)
      });
      ...
      it.only('', async () => {
        ...
        const accumulatedSTKRPerShare = await _mockContractHelper.updateAccumulatedSTKRperShare(
          ...
          const rewardClientFixedPoint = (clientComputation.multipliedBy(new BigNumber(e12)).decimalPlaces(0,1)).toFixed()
            console.log(rewardClientFixedPoint)
        expect(accumulatedSTKRPerShare).to.equal('100000000000000000000')
